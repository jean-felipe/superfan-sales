                  ...
                  <div class="field-body">
                    ...
                    <div class="control is-expanded">
                      <input class="input is-marginless" type="text" placeholder="Nome do negócio" 
                        v-model="newTable.name" required>
                    </div>
                  </div>
                    ...
                    <div class="control is-expanded">
                      <div class="is-fullwidth">
                        <input class="input is-marginless" type="number" placeholder="Nome do negócio" 
                          v-model="newTable.places" required>
                      </div>
                    </div>
                  </div>
                  </div>
...
export default {
  ...
  methods: {
    handleSubmit() {
      ...
      axios.post('/api/v1/tables', this.newTable)
        .then((response) => {
          console.log(response)
          // // this.sendImages(response.data.id)

          this.$swal("Parabéns!", "Mesa cadastrada com sucesso!", "success")
            .then(() => {
